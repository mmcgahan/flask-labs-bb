...
from flask_labs.models import User, Post, Tag, Section
...
def newCategory(blogid, username, password, category):
    ...
    if user is None or not user.check_password(password):
        raise Fault("invalid_user",
                    "Invalid username/password, please try again.")
    section = Section.query.filter(Section.name == category['name']).first()
    if section is None:
...
def getPostCategories(post_id, username, password):
    section = Post.query.get(post_id).section
